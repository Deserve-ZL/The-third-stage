...
export default class ApplyDemo extends React.Component {
  constructor(props) {
    super(props);
    this.columns = [
      {
  ...
  //编辑
  handlePopup() {
    var tds = window.event.target.parentNode.parentNode.parentNode.children
    // tds[0].
    console.log(tds)
    this.setState({
      visible: true,
  ...
  handleAdd = () => {
    ...
    const newData = {
      key: this.state.countNum,
      name: `Edward King ${this.state.countNum}`,
      age: 32,
      address: `London, Park Lane no. ${this.state.countNum}`,
    };
    this.setState({
